...
import { getProject, getLeadsByProject } from "@/lib/queries";
...
export default async function ProjectDetail({ params }) {
  ...
  const recentLeads = await getLeadsByProject(p.id, { limit: 10 });
  ...
  const stats = [
    ...
    { label: "Conversion", value: p.leads ? `${Math.round((appts / p.leads) * 100)}%` : "0%" },
  ];
